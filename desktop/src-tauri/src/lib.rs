...
fn preferred_model() -> &'static str {
    ...
    match fitting.len() {
        0 => FALLBACK_MODEL,
        1 => fitting[0],
        2 => fitting[0], // only two fit → pick the smaller one
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    let models_to_pull: Vec<&str> = if fitting.is_empty() {
        ...
        // Include fallback as a safety net, plus all fitting Qwen3.5 models
    ...
    for (i, model) in models_to_pull.iter().enumerate() {
        ...
        if !ollama_has_model(model).await {
            ...
            if let Err(e) = pull_model(model).await {
                // Log the error but keep going — one failed pull shouldn't
                // block the app if other models succeed.
